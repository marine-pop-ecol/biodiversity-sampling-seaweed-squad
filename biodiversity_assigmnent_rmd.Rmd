...
seaweeds = perc %>%
  ...
  select(c(unique_ID, quadrat, per_upright, per_crust)) %>%
  arrange(unique_ID)
...
ggplot(data = Data, aes(x=per_upright, y =ShannonIndex))+ 
  geom_point()+
  geom_smooth(method=lm, se=FALSE)

# visualizing crust percent cover and diversity
ggplot(data = Data, aes(x=per_crust, y =ShannonIndex))+ 
  geom_point()+
  geom_smooth(method=lm, se=FALSE)

# visualizing upright percent cover and richness
ggplot(data = Data, aes(x=per_upright, y =rich))+ 
  geom_point()+
  geom_smooth(method=lm, se=FALSE)

# visualizing crust percent cover and richness
ggplot(data = Data, aes(x=per_crust, y =rich))+ 
  geom_point()+
  geom_smooth(method=lm, se=FALSE)
...
check_model(upright_diversity) # our model isn't perfect but it'll do donkey, it'll do 
...
check_model(crust_diversity) # our model isn't perfect but it'll do donkey, it'll do 
...
check_model(upright_rich) # our model isn't perfect but it'll do donkey, it'll do 

# simple linear regression for crust and richness
crust_rich = lm(rich ~ per_crust, data = Data)
summary(crust_rich) # not significant
check_model(crust_rich) # our model isn't perfect but it'll do donkey, it'll do 
